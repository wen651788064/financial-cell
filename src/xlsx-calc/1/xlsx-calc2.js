...
// +---------------------+
            ...
            //XW:end todo: 让tt帮忙放到其他文件中去。
            var ROUND = function (value, places) {
                var n = value;
                var d = places;
                return Math.round(n * Math.pow(10, d)) / Math.pow(10, d);
            };
            function checkType(params, type_list, error_return){
                for (var i=0; i<type_list; i++){
                    if (typeof params == type_list[i]){
                        return true
                    }
                }
                return error_return
            }
            function FLOORMATH(number, significance, mode) {
                // function getExcelFunction(){
                //
                // checkTyp(number, 'number', '#VALUE'); checkType(number, ['number','string'], '#NUM')
                // XW: 自己写的函数
                checkType(number, ['number'], error.value)
...
/// C# implementation found at http://weblogs.asp.net/esanchez/archive/2010/07/29/a-quick-and-dirty-implementation-of-excel-norminv-function-in-c.aspx
            ...
            function irr(range, guess) {
                if (typeof guess == 'string' && guess.indexOf('%') >= 0){
                    guess = guess.replace('%', '')
                    guess = parseFloat(guess) / 100;
                }
                var min = -2.0;
                var max = 1.0;
                var n = 0;
            ...
            function vlookup(key, matrix, return_index, cumulative) {
        ...
        /***/ (function (module, exports, __webpack_require__) {
            ...
            // import {is_valid} from "./src/formula_check.js"
                    ...
                    try {
                        ...
                        if (!is_valid(cell)){
                            ...
                            if (cell.f.indexOf('{') >=0 && cell.f.indexOf('}')>=0 && cell.f.indexOf("'{") <0){
                                ...
                                formulas[i].cell = trans_params(cell)
